...
public class MissedCallsHamcrestTests {
    ...
    public void addMissedCallTest() {
        LocalDateTime date = LocalDateTime.of(2021, 1, 13, 22, 1, 45, 0);
        ...
        assertThat(
                ...
                allOf(
                        startsWith("Пропущенные звонки:\n"),
                        both(
                                containsString(now.format(dtFormatter))).
                                and(containsString(date.format(dtFormatter))
                                ),
                        endsWith("\n")
                )
        );
        assertThat(isBothAdded, is(true));
    }
    ...
    public void setPhoneNumberTest(PhoneBook phoneBook) {
        ...
        //assert
        assertThat(
                systemUnderTest.toString(phoneBook),
                equalTo(expected)
        );
        assertThat(isAllAdded, is(true));
    }
